...
const DEFAULT_VALUE_SETTINGS = {
  ...
  ANSWER_REVEAL_DELAY: 0.6 * 1000, // milliSec damage-flash 0.6s ease-outと合わせる
...
const CARD_PATH_SETTINGS = {
  RED_CARD: `${BG_IMG_PATH_SETTINGS.IMG_DIRECTORY_PATH}red${BG_IMG_PATH_SETTINGS.CARD_IMG_EXT}`,
  BLUE_CARD: `${BG_IMG_PATH_SETTINGS.IMG_DIRECTORY_PATH}blue${BG_IMG_PATH_SETTINGS.CARD_IMG_EXT}`,
};
...
const getCardPath = (value) => {
  return `${BG_IMG_PATH_SETTINGS.IMG_DIRECTORY_PATH}${value}${BG_IMG_PATH_SETTINGS.CARD_IMG_EXT}`;
};
...
const closeIntercalaryCard = () => {
  Cards.calculating.intercalary.setAttribute("src", `${BG_IMG_PATH_SETTINGS.IMG_DIRECTORY_PATH}y.png`);
};
...
const UI = {
  ...
  buttons: {
    ...
    monthAdjustbc: document.querySelectorAll(".month-adjust-button-container"),
  },
  status: {
...
let hudState = { ...INITIAL_HUD_STATE };

const initializeHudState = () => {
  hudState = { ...INITIAL_HUD_STATE };
  hudState.misses = { ...INITIAL_HUD_STATE.misses };
};
...
let calendarState = { ...INITIAL_CALENDAR_STATE };

const initializeCalendarState = () => {
  calendarState = { ...INITIAL_CALENDAR_STATE };
};
...
let questionState = { ...INITIAL_QUESTION_STATE };

const initializeQuestionState = () => {
  questionState = { ...INITIAL_QUESTION_STATE };
};
...
let Answer = { ...INITIAL_ANSWER_STATE };

const initializeAnswerState = () => {
  Answer = { ...INITIAL_ANSWER_STATE };
};
...
let markingState = { ...INITIAL_MARKING_STATE };

const initializeMarkingState = () => {
  markingState = { ...INITIAL_MARKING_STATE };
};
...
let usedCards = { ...INITIAL_USEDCARDS_STATE };

const initializeUsedCards = () => {
  usedCards = { ...INITIAL_USEDCARDS_STATE };
};
...
let timerState = { ...INITIAL_TIMER_STATE };

const initializeTimerState = () => {
  timerState = { ...INITIAL_TIMER_STATE };
  timerState.display = { ...INITIAL_TIMER_STATE.display };
};
...
const outPutResult = () => {
  const prefix = markingState.correct ? "%c○ CORRECT\n" : markingState.timeIsUp ? "%c× TimeIsUp\n" : "%c× INCORRECT\n";
  ...
  const century = `Century:${calendarState.centuryData}  CA=${Answer.century}`;
  ...
  const intercalary = `Intercalary:   ${Answer.intercalaryDisplay}`;
...
const gameEnd = () => {
  ...
  UI.result.totalResult.innerHTML = `
    ...
    CorrectRate:<br>
    ${formatToFixAndPad(correctRate)}%<br>
    (DayAnswer)
  `;
...
const afterSelectDayCard = () => {
  ...
  drawUIValues();
  openDayCardAnswer();
  Cards.current.day.style.opacity = OPACITY.FULL; // 強調表示
  calculateAverageTime();
...
const openOtherGamesDialog = () => {
  ...
  UI.buttons.gameA.onclick = () => {
    console.log("done.");
    // タブで開く
  };
  UI.buttons.gameB.onclick = () => {
    console.log("done.");
    // タブで開く
  };
